 ...
 *
 * Optimistic update: o cache da sessão (`execucaoSessaoKeys.detail(sessaoId)`) é
 * atualizado antes da resposta do backend para que a UI reaja instantaneamente.
...
export function useRegistrarSerie(sessaoId: string) {
  ...
  const queryKey = execucaoSessaoKeys.detail(sessaoId);
