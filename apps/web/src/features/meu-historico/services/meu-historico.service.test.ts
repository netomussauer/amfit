...
import { meuHistoricoService } from './meu-historico.service';
...
describe('meuHistoricoService.listar', () => {
  beforeEach(() => {
    mockedGet.mockReset();
  });

  it('busca /alunos/me/sessoes com page e per_page corretos', async () => {
    mockedGet.mockResolvedValueOnce({
      data: { data: [sessaoResumoFixture], pagination: { total: 1, page: 1, per_page: 20 } },
    });

    const resultado = await meuHistoricoService.listar({ page: 1, perPage: 20 });
  ...
  it('lanca erro de validacao quando a resposta nao bate com o schema', async () => {
    mockedGet.mockResolvedValueOnce({
      data: {
        data: [{ ...sessaoResumoFixture, status: 'STATUS_INVALIDO' }],
        pagination: { total: 1, page: 1, per_page: 20 },
      },
    });

    await expect(meuHistoricoService.listar({ page: 1, perPage: 20 })).rejects.toThrow();
  });
});

describe('meuHistoricoService.buscar', () => {
  beforeEach(() => {
    mockedGet.mockReset();
  });
  ...
  it('busca /sessoes/:id e retorna os dados quando a resposta e valida', async () => {
    ...
    const resultado = await meuHistoricoService.buscar(sessaoFixture.id);
  ...
  it('lanca erro de validacao quando data_execucao nao bate com o formato YYYY-MM-DD', async () => {
    ...
    await expect(meuHistoricoService.buscar(sessaoFixture.id)).rejects.toThrow();
  });
});
