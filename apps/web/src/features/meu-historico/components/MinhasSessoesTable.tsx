...
export function MinhasSessoesTable() {
  const [page, setPage] = useState(1);

  const { data, isLoading, isError, refetch, isFetching } = useMinhasSessoes({
    page,
    perPage: PER_PAGE,
  });
        ...
        {isLoading ? (
          ...
          <div className="px-4 py-12 text-center">
            <p className="text-sm text-[--color-text-muted]">
              Você ainda não executou nenhuma sessão de treino.
            </p>
            <p className="mt-1 text-xs text-[--color-text-muted]">
              Suas sessões aparecem aqui assim que você iniciar um treino.
            </p>
          </div>
        ) : (
          <table
            className="w-full divide-y divide-[--color-border]"
            aria-label="Histórico das minhas sessões"
          >
...
function SessaoRow({ sessao }: { sessao: SessaoResumoResponse }) {
